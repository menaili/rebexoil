...
class Induscity_Portfolio {
	...
	public function register_post_type() {
		...
		$portfolio_permalink = empty( $permalinks['portfolio_base'] ) ? _x( 'portfolio', 'slug', 'induscity' ) : $permalinks['portfolio_base'];
		$portfolio_page_id   = get_option( $this->option . '_page_id' );
		$portfolio_category_base = empty( $permalinks['portfolio_category_base'] ) ? _x( 'portfolio-category', 'slug', 'induscity' ) : $permalinks['portfolio_category_base'];

		register_post_type( $this->post_type, array(
			'description'         => esc_html__( 'Portfolio Items', 'induscity' ),
			'labels'              => array(
				'name'                  => esc_html__( 'Portfolio', 'induscity' ),
				'singular_name'         => esc_html__( 'Project', 'induscity' ),
				'menu_name'             => esc_html__( 'Portfolio', 'induscity' ),
				'all_items'             => esc_html__( 'All Projects', 'induscity' ),
				'add_new'               => esc_html__( 'Add New', 'induscity' ),
				'add_new_item'          => esc_html__( 'Add New Project', 'induscity' ),
				'edit_item'             => esc_html__( 'Edit Project', 'induscity' ),
				'new_item'              => esc_html__( 'New Project', 'induscity' ),
				'view_item'             => esc_html__( 'View Project', 'induscity' ),
				'search_items'          => esc_html__( 'Search Projects', 'induscity' ),
				'not_found'             => esc_html__( 'No Projects found', 'induscity' ),
				'not_found_in_trash'    => esc_html__( 'No Projects found in Trash', 'induscity' ),
				'filter_items_list'     => esc_html__( 'Filter projects list', 'induscity' ),
				'items_list_navigation' => esc_html__( 'Project list navigation', 'induscity' ),
				'items_list'            => esc_html__( 'Projects list', 'induscity' ),
			),
			'supports'            => array(
				'title',
				'editor',
		...
		register_taxonomy( $this->taxonomy_type, $this->post_type, array(
			'hierarchical'      => true,
			'labels'            => array(
				'name'                  => esc_html__( 'Portfolio Categories', 'induscity' ),
				...
				'items_list_navigation' => esc_html__( 'Category list navigation', 'induscity' ),
				'items_list'            => esc_html__( 'Category list', 'induscity' ),
			),
			'public'            => true,
			'show_ui'           => true,
			'show_in_nav_menus' => true,
	...
	public function edit_admin_columns( $columns ) {
		...
		$columns['title'] = esc_html__( 'Project', 'induscity' );
	...
	public function settings_api_init() {
		add_settings_section(
			'induscity_portfolio_section',
			'<span id="portfolio-options">' . esc_html__( 'Portfolio', 'induscity' ) . '</span>',
		...
		add_settings_field(
			$this->option,
			'<span class="portfolio-options">' . esc_html__( 'Portfolio Projects', 'induscity' ) . '</span>',
		...
		if ( get_option( $this->option ) ) {
			...
			add_settings_field(
				...
				'<span class="portfolio-options">' . esc_html__( 'Portfolio page', 'induscity' ) . '</span>',
			...
			add_settings_field(
				...
				'<label for="portfolio_items_per_page">' . esc_html__( 'Portfolio items show at most', 'induscity' ) . '</label>',
			...
			add_settings_section(
				'induscity_portfolio_section',
				'<span id="portfolio-options">' . esc_html__( 'Portfolio Item Permalink', 'induscity' ) . '</span>',
			...
			add_settings_field(
				...
				'<label for="portfolio_category_slug">' . esc_html__( 'Portfolio Category base', 'induscity' ) . '</label>',
	...
	public function permalink_section_html() {
		...
		$portfolio_page_id = get_option( $this->option . '_page_id' );
		$base_slug         = urldecode( ( $portfolio_page_id > 0 && get_post( $portfolio_page_id ) ) ? get_page_uri( $portfolio_page_id ) : _x( 'portfolio', 'Default slug', 'induscity' ) );
		$portfolio_base    = _x( 'portfolio', 'Default slug', 'induscity' );
		...
		?>
		<p>
			<?php esc_html_e( 'Use these settings to control the permalink used specifically for portfolio.', 'induscity' ); ?>
		</p>

		<table class="form-table induscity-portfolio-permalink-structure">
			<tbody>
			<tr>
				<th>
					<label><input name="portfolio_permalink" type="radio"
								  ...
								  class="induscity-portfolio-base" /> <?php esc_html_e( 'Default', 'induscity' ); ?>
					</label>
				</th>
				<td>
				...
				<tr>
					<th>
						<label><input name="portfolio_permalink" type="radio"
									  ...
									  class="induscity-portfolio-base" /> <?php esc_html_e( 'Portfolio base', 'induscity' ); ?>
						</label>
					</th>
					<td>
			...
			<tr>
				<th>
					<label><input name="portfolio_permalink" type="radio"
								  ...
								  class="induscity-portfolio-base" /> <?php esc_html_e( 'Portfolio category with type', 'induscity' ); ?>
					</label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label><input name="portfolio_permalink" id="induscity_portfolio_custom_selection" type="radio"
								  value="custom" <?php checked( in_array( $portfolio_permalink, $structures ), false ); ?> /> <?php esc_html_e( 'Custom Base', 'induscity' ); ?>
					</label>
				</th>
				<td>
					<code><?php echo esc_html( home_url() ); ?></code>
					<input name="portfolio_permalink_structure" id="induscity_portfolio_permalink_structure" type="text"
		...
		<script type="text/javascript">
			jQuery( function () {
				jQuery( 'input.induscity-portfolio-base' ).change( function () {
					jQuery( '#induscity_portfolio_permalink_structure' ).val( jQuery( this ).val() );
				} );
				jQuery( '.permalink-structure input' ).change( function () {
					jQuery( '.induscity-portfolio-permalink-structure' ).find( 'code.non-default-example, code.default-example' ).hide();
					if ( jQuery( this ).val() ) {
						jQuery( '.induscity-portfolio-permalink-structure code.non-default-example' ).show();
						jQuery( '.induscity-portfolio-permalink-structure input' ).removeAttr( 'disabled' );
					} else {
						jQuery( '.induscity-portfolio-permalink-structure code.default-example' ).show();
						jQuery( '.induscity-portfolio-permalink-structure input:eq(0)' ).click();
						jQuery( '.induscity-portfolio-permalink-structure input' ).attr( 'disabled', 'disabled' );
					}
				} );
				jQuery( '.permalink-structure input:checked' ).change();
				jQuery( '#induscity_portfolio_permalink_structure' ).focus( function () {
					jQuery( '#induscity_portfolio_custom_selection' ).click();
				} );
			} );
		</script>
		<?php
	...
	public function enable_field_html() {
		...
		<label for="<?php echo esc_attr( $this->option ); ?>">
			...
			<?php esc_html_e( 'Enable Portfolio Projects for this site.', 'induscity' ); ?>
		</label>
	...
	public function per_page_field_html() {
		...
		<label for="portfolio_posts_per_page">
			<input name="<?php echo esc_attr( $name ) ?>" id="portfolio_items_per_page" type="number" step="1" min="1"
				   value="<?php echo esc_attr( get_option( $name, '12' ) ) ?>" class="small-text" />
			<?php _ex( 'items', 'Portfolio items per page', 'induscity' ) ?>
		</label>
	...
	public function portfolio_category_slug_field_html() {
		...
		<input name="portfolio_category_slug" id="portfolio_category_slug" type="text"
			   ...
			   placeholder="<?php echo esc_attr( _x( 'portfolio-category', 'Category base', 'induscity' ) ) ?>"
		...
		if ( isset( $_POST['portfolio_permalink'] ) ) {
			...
			if ( 'custom' === $portfolio_permalink ) {
				...
				if ( '%portfolio_category%' == $portfolio_permalink ) {
					$portfolio_permalink = '/' . _x( 'portfolio', 'slug', 'induscity' ) . '/' . $portfolio_permalink;
				}
			...
			$portfolio_page_id   = get_option( $this->option . '_page_id' );
			$portfolio_permalink = ( $portfolio_page_id > 0 && get_post( $portfolio_page_id ) ) ? get_page_uri( $portfolio_page_id ) : _x( 'portfolio', 'Default slug', 'induscity' );
	...
	public function portfolio_post_type_link( $permalink, $post ) {
		...
		if ( ! empty( $terms ) ) {
			...
			$portfolio_category = _x( 'uncategorized', 'slug', 'induscity' );
		}
	...
	public function breadcrumb( $args ) {
		...
		if ( is_post_type_archive( 'portfolio' ) ) {
			...
			if ( $portfolio_page_id && get_post( $portfolio_page_id ) ) {
				...
				$args['labels']['archive'] = _x( 'Portfolio', 'Portfolio post type breadcrumb', 'induscity' );
			}
		}
	...
	public function archive_title( $title ) {
		if ( is_post_type_archive( 'portfolio' ) ) {
			...
			if ( $portfolio_page_id && get_post( $portfolio_page_id ) ) {
				...
				$title = _x( 'Portfolio', 'Portfolio post type breadcrumb', 'induscity' );
			}
		}
