...
if( ! class_exists('Induscity_Services_Menu_Widget')) {
	class Induscity_Services_Menu_Widget extends WP_Widget {
		/**
		 ...
		 * Constructor
		 *
		 * @return Induscity_Services_Menu_Widget
		 */
		function __construct() {
			$this->defaults = array(
				...
				'order'     => 'DESC',
				'orderby'   => 'date',
				'cat_title' => esc_html__( 'See All Services', 'induscity' ),
			);

			parent::__construct(
				'services-menu-widget',
				esc_html__( 'Induscity - Services Menu', 'induscity' ),
				array(
					'classname'   => 'services-menu-widget',
					'description' => esc_html__( 'Advanced services menu widget.', 'induscity' )
				)
			);
		}
		...
		function widget( $args, $instance ) {
            ...
            <ul class="menu service-menu">
                <li>
                    <a href="<?php echo esc_url( $term_link ); ?>"><i
                                class="fa fa-long-arrow-right"></i><?php echo esc_html( $cat_title ) ?>
                    </a></li>
				<?php
				...
				while ( $query->have_posts() ) : $query->the_post();
                    ...
                    <li class="menu-item <?php echo esc_attr( $class ) ?>">
                        <a href="<?php the_permalink(); ?>"><i class="fa fa-long-arrow-right"></i><?php the_title(); ?>
                        </a>
                    </li>
				<?php
				endwhile;
		...
		function form( $instance ) {
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'cat_title' ) ); ?>"><?php esc_html_e( 'Category Title', 'induscity' ); ?></label>
            ...
            <p>
                ...
                <label for="<?php echo esc_attr( $this->get_field_id( 'show_cat' ) ); ?>"><?php esc_html_e( 'Show Services are the same Categories', 'induscity' ); ?></label>
            </p>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>"><?php esc_html_e( 'Order By', 'induscity' ); ?></label>
                <select name="<?php echo esc_attr( $this->get_field_name( 'orderby' ) ); ?>"
					...
					<?php foreach ( $order_by = $this->service_cat_order_by() as $name => $value ) : ?>
                        <option value="<?php echo esc_attr( $name ) ?>" <?php selected( $name, $instance['orderby'] ) ?>><?php echo esc_attr( $value ) ?></option>
					<?php endforeach; ?>
                </select>
            </p>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"><?php esc_html_e( 'Order', 'induscity' ); ?></label>
                <select name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>"
					...
					<?php foreach ( $order_by = $this->service_cat_order() as $name => $value ) : ?>
                        <option value="<?php echo esc_attr( $name ) ?>" <?php selected( $name, $instance['order'] ) ?>><?php echo esc_attr( $value ) ?></option>
					<?php endforeach; ?>
                </select>
            </p>
		...
		function service_cat_order_by() {
			$order_by = array(
				...
				'ID'   => esc_html__( 'ID', 'induscity' ),
				'name' => esc_html__( 'name', 'induscity' ),
			);
		...
		function service_cat_order() {
			$order = array(
				'DESC' => esc_html__( 'DESC', 'induscity' ),
				'ASC'  => esc_html__( 'ASC', 'induscity' ),
			);
